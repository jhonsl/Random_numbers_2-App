...
public class RandomController {
    ...
    @PostMapping("/range")
    public Mono<Random> postInRange(@RequestBody RequestDTO request) {
        return Mono.just(new Random()).map(entity -> {
            entity.setDate(new Date());
            var list = IntStream.range(request.getNumero1(), request.getNumero2() + 1)
                .mapToObj(num -> String.valueOf(num))
                .collect(Collectors.joining(","));
            entity.setOrginalList(list);
            return entity;
        }).map(entity -> {
            var list = Stream.of(entity.getOrginalList().split(","))
                .map(dato -> dato.trim()).collect(Collectors.toList());
            
            Collections.shuffle(list);
        
            var numerosAleatorios = list.stream().collect(Collectors.joining(","));
            entity.setRandomList(numerosAleatorios);
